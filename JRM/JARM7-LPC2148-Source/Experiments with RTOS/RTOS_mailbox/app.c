...
static  void  AppTaskStart (void *p_arg)
{
    ...
    AppMbox = OSMboxCreate((char*)post_msg);                       /* Create MBOX for communication between Kbd and Start      */	

    while (1) {
    OSMboxPost(AppMbox, (char*)post_msg);		
	OSTimeDly(50);
    }
...
static  void  AppTaskLed (void *p_arg)
{
  ...
  (void)p_arg;
    ...
    while (1) {
    msg = OSMboxAccept(AppMbox);
	if(*msg == 'T')
	{
	 lcd_clear();
	 lcd_putstring(0,"MSG_RECIEVED");			
	}
	OSTimeDly(50);
    }
}
